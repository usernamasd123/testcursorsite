export default function Card({ title, description, features, contact }) {
  const handleContact = () => {
    const message = `Заинтересован в сотрудничестве с ${title}`;
    const telegramUrl = `https://t.me/share/url?url=${encodeURIComponent(message)}&text=${encodeURIComponent(message)}`;
